...
class IsarFinanzasRepository implements FinanzasRepository {
  ...
  @override
  Future<List<IncomeRecord>> getUnsynchronizedIncomes() async {
    final all = await _isar.isarIncomeRecords.where().findAll();
    return all
        .where((r) => !r.synced)
        .map((r) => r.toEntity())
  ...
  @override
  Future<void> markIncomeSynced(String id, String remoteId) async {
    final isarId = int.tryParse(id);
    if (isarId == null) return;
    await _isar.writeTxn(() async {
      final record = await _isar.isarIncomeRecords.get(isarId);
      if (record == null) return;
      record
        ..synced = true
        ..remoteId = remoteId
        ..syncDate = DateTime.now();
      await _isar.isarIncomeRecords.put(record);
    });
  }

  @override
  Future<List<GeneralExpenseRecord>> getUnsynchronizedExpenses() async {
    final all = await _isar.isarGeneralExpenseRecords.where().findAll();
    return all
        .where((r) => !r.synced)
        .map((r) => r.toEntity())
        .toList(growable: false);
  ...
  Future<void> markExpenseSynced(String id, String remoteId) async {
    ...
    if (isarId == null) return;
    await _isar.writeTxn(() async {
      final record = await _isar.isarGeneralExpenseRecords.get(isarId);
      if (record == null) return;
      record
        ..synced = true
        ..remoteId = remoteId
        ..syncDate = DateTime.now();
      await _isar.isarGeneralExpenseRecords.put(record);
    });
  }
}
